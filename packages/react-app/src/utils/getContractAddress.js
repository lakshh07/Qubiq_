...
import { addresses, chainlinkOraclesRinkeby } from "./Addresses";
...
export async function getTokenValue(token, provider) {
  if (typeof window.ethereum !== "undefined") {
    ...
    const routerContract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
...
export async function getUserHoldingForBasket(defaultAccount, _basketId, token, provider) {
  if (typeof window.ethereum !== "undefined") {
    const signer = provider.getSigner();
    const routerContract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
    ...
    try {
      ...
      const balance = await routerContract.getUserHoldingForToken(defaultAccount, _basketId, token);
...
export async function getApproval(token, provider, address) {
  if (typeof window.ethereum !== "undefined") {
    ...
    const approvalAmount = await contract.allowance(address, subscribeContractAddress);
    // console.log("balance of user: ", userBalance);
    // console.log("approval for the token: ", approvalAmount);
    try {
      const transaction = await contract.approve(subscribeContractAddress, amount);
...
export async function createBasket(
  ...
  if (typeof window.ethereum !== "undefined") {
    const signer = provider.getSigner();
    const contract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
    try {
      const transaction = await contract.createBasket(tokens, weights, id);
      const data = await transaction.wait();
      const event = await data.events.find(event => event.event === "basketCreated");
      // console.log("event.args");
      // console.log(event);
      const [creator, basket] = await event.args;
      // console.log(creator, basket);

      alert(`basket created by ${creator}, ${basket}`);
      client
...
export async function addFunds(provider, _basketID, _tokenIn, _amount) {
  if (typeof window.ethereum !== "undefined") {
    const signer = provider.getSigner();
    const contract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
...
export async function partialExit(provider, _basketID, _tokenIn, _amount) {
  if (typeof window.ethereum !== "undefined") {
    const signer = provider.getSigner();
    const contract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
...
export async function exitBasket(provider, _basketID, _tokenOut, defaultAccount, navigate) {
  if (typeof window.ethereum !== "undefined") {
    const signer = provider.getSigner();
    const contract = new ethers.Contract(subscribeContractAddress, SubscribeBasket.abi, signer);
